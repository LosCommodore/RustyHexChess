...
| 13 | `game.rs:208` | Naming, not behaviour. `move_leaves_king_in_check` was renamed `is_pinned`, but it answers a broader question: it executes the move and asks whether the mover's own king is then attacked. That covers moving a pinned piece, moving the king *into* check, and failing to answer an existing check — only the first is a pin. The old name was accurate. |
